...
/* Sets up two buffers, one for storing data, one for processing */
...
********************************************************************************
* Summary:
*  This function feeds the data to the imagimob pre-processor and returns the
*  processed data.
*
* Parameters:
*  preprocessed_data: Stores the data once processed
*
*******************************************************************************/
bool pdm_preprocessing_feed(float *preprocessed_data)
{
    float val = 0.0f;
    bool complete_inference = PDM_PROCESSING_INCOMPLETE;
    ...
    for (uint32_t index = 0; index < FRAME_SIZE ; index ++)
    {
        ...
        IMAI_PDM_enqueue(&val);

        /* Check if there is pre-processed data ready for inferencing */
        if (IMAI_PDM_dequeue(preprocessed_data) == IMAI_PDM_RET_SUCCESS)
        {
            complete_inference = PDM_PROCESSING_COMPLETE;
        }
    }
